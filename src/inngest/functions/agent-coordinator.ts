  ...
  async ({ event, step }) => {
    try {
      ...
      if (assignments.length === 0) {
        return {
          message: 'No matching agents for pending tasks',
      ...
      await step.run('assign-tasks', async () => {
        const updates = [];
      ...
      console.error('[Agent Coordinator] Error:', errorMessage);
      return {
        message: 'Coordinator error - will retry',
        error: errorMessage,
